...
def generate_ens(ddim_steps):
    ...
    min_normal = np.load(f"{path_tmp}/state_min.npy")[list_idx]
    all_data=[]
    member_data=[]
    batch = 1
    val_dataset = SR3_Dataset(var=variable_name)
    ...
    with torch.no_grad():
        ...
        print("new_data shape:", new_data.shape, "mmm: ", np.min(new_data), np.mean(new_data), np.max(new_data))
        ...
        print("new_member_data shape:", new_member_data.shape, "mmm: ", np.min(new_member_data), np.mean(new_member_data), np.max(new_member_data))

    t = range(0, new_data.shape[0])
    y = range(0, new_data.shape[1])
    x = range(0, new_data.shape[2])
